...
def infosu_mydegrees_redirect():
    ...
    request = urllib.request.Request(form_page, headers = header_values)
...
def first_page_set_cookie(form_list):
    ...
    request = urllib.request.Request(mydegrees_url, form_data, headers= header_values)
    response = opener.open(request)

    header_values['Referer'] = "https://mydegrees.oregonstate.edu/SD_LoadFrameForm.html"

    form_data = urllib.parse.urlencode({'SERVICE':'SCRIPTER','SCRIPT':'SD2STUCON'})
    request = urllib.request.Request(mydegrees_url, form_data, headers = header_values)
    response = opener.open(request)
